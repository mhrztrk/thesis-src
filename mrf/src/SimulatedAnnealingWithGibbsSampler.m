function [labels_GlbMin] = SimulatedAnnealingWithGibbsSampler(linelist, iLabels, nstep, nlevel, iTemp, mrf, Cliques)
    ...
    while(deltaE >= 0.000 && K <= nlevel)
        ...
        for k=1:nstep
            ...
            for i=1:(nconn-(ngrp-1))
                ...
                labels_l(i:(i+ngrp-1),:) = labels_allp;
                
                for m=1:(2^ngrp)
                    prob(m) = 0;
                    for n=1:ngrp
                        prob(m) = prob(m) + exp(-MRFLocalEnergy(i + n - 1 , linelist, labels_l(:,m), Cliques, mrf)/T);
                    end
                end
