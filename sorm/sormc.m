...
function [CC, ST, pred] = sormc(I, d)
    ...
    min_dist = 11;         % minimum distance between the nodes
    min_sample_th = 5;     % minimum number of samples 
    ...
    [CC ID CM] = KMedians(m, CCi, min_dist, min_sample_th, I);
    ...
    saveas(gcf, sprintf('Minimum Spanning Tree (d=%2d,d_min=%2d).jpg', dist, min_dist));
...
%
function [CC ID CM]= KMedians(m, sp, min_dist, min_sample_th, I)

    c = sp;
    global dist;
    
    [imH imW imD] = size(I);
    Iv = reshape(I, imH*imW, imD);
    
    % Show initial centroid locations
    figure;imshow(I);hold on
    scatter(sp(:,2),sp(:,1),'b','o', 'filled');     
    
    Mx = meshgrid(1:imH,1:imW);
    My = Mx';
    Mg = [Mx(:) My(:)];
    
    temp=zeros(size(Mg, 1),1);   % initialize as zero vector
...
%         close gcf;  
        ...
        if (~isempty(find(D < min_dist, 1)))
            ...
            while(1)
                ...
                if (~isinf(v))
                    ...
                    c = c_pruned(c_pruned(:,1)~=0 & c_pruned(:,2)~=0,:);
                    break
                end
        ...
        figure;imshow(I,[]);hold on;scatter(c(:,1),c(:,2),'*');
...
function ShowTreeOnImage(I, M, CC)
    ...
    for i=1:w
        ...
        if ~isempty(ind)
            for j=1:size(ind,1)
                line([CC(i,2) CC(ind(j),2)],[CC(i,1) CC(ind(j),1)],'LineWidth',3);
            end
        end
    end
end
